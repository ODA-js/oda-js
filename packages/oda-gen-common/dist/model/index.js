    ...
    applyHooks() {
        ...
        for (let i = 0, len = modelHooks.length; i < len; i++) {
            ...
            for (let j = 0, jLen = hookList.length; j < jLen; j++) {
                let key = hookList[j];
                const resoler = lodash_2.get(this.resolvers, key);
                lodash_2.set(this.resolvers, key, modelHooks[i][key](resoler));
            }
        }
    }
    fixSchema() {
...
function cloneCustomizer(value) {
    if (value && value.kind && needToFix(value)) {
        return Object.assign({}, value, { kind: value.kind.replace(/Extension/i, 'Definition') });
    }
