...
function find(array, item) {
    ...
    if (result === -1 && typeof item === 'object') {
        array.some((f, i) => {
            const res = lodash_1.isEqual(f, item);
            if (res) {
                result = i;
            }
            return res;
...
exports.find = find;
function arrayItemOperation(inp) {
...
exports.arrayItemOperation = arrayItemOperation;
...
function removeIfExists(result, current) {
    if (Array.isArray(current)) {
        current.forEach(item => {
...
function processArrayItem(result, value) {
...
function processArray(result, current) {
    if (Array.isArray(current)) {
        current.forEach(item => {
                ...
                if (!Array.isArray(result)) {
                    ...
                    for (let j = 0, kLen = keys.length; j < kLen; j++) {
                        ...
                        if (current.hasOwnProperty(key)) {
                            ...
                            let rv = get_1.default(result, key);
                            if (result.hasOwnProperty(key) &&
                                (typeof rv === 'object' && rv !== null)) {
                                set_1.default(result, key, deepMerge(rv, cv));
                            }
                            else {
